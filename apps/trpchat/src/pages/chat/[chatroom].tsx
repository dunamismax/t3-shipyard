...
const ChatroomPage = () => {
  ...
  useEffect(() => {
    if (typeof server === "string" && typeof username === "string" && typeof chatroom === "string") {
      ...
      <main className="flex h-screen flex-col bg-gray-900 text-white">
          ...
          <div className="flex flex-col gap-2">
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.username === username ? "justify-end" : "justify-start"}`}>
                <div className={`rounded-lg px-4 py-2 ${msg.username === username ? "bg-blue-600" : "bg-gray-700"}`}>
        ...
        <div className="p-4 bg-gray-800">
          <div className="flex gap-2">
            ...
            />
            <button onClick={sendMessage} className="rounded-full bg-blue-600 px-4 py-2 font-bold text-white">
              Send
            </button>
          </div>
        </div>
